    ...
    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                ...
                Forms\Components\Section::make('Lien vers le contenu (optionnel)')
                    ->description('Si cette notification concerne un contenu spécifique (article, forum, etc.)')
                    ->schema([
                            ...
                            ->schema([
                                Forms\Components\Select::make('category')
                                    ...
                                    ->afterStateUpdated(function ($state, callable $get, callable $set) {
                                        // Auto-suggérer le type de contenu
                                        $typeMapping = [
                                            'content' => 'article',
                                            'forum' => 'forum_reply',
                                            'cycle' => 'cycle',
                                        ...
                                        if (isset($typeMapping[$state])) {
                                            $set('related_type', $typeMapping[$state]);
                                        }
                                    }),

                                Forms\Components\TextInput::make('related_id')
                                    ->label('ID du contenu (ex: ID de l\'article)')
                                    ->numeric()
                                    ->minValue(1)
                                    ->placeholder('Laisser vide si pas de contenu spécifique')
                                    ->reactive()
                                    ->afterStateUpdated(function ($state, callable $get, callable $set) {
